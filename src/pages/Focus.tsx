...
export default function Focus() {
  ...
  const adjustTime = (minutes: number) => {
    ...
    const newDuration = Math.max(60, Math.min(120 * 60, currentDuration + minutes * 60));
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center pt-20 pb-6 px-4 bg-aura-black overflow-hidden">
      ...
      >
        ...
        <div className="relative flex items-center justify-center w-64 h-64 md:w-80 md:h-80 mb-8 md:mb-10 group">
          ...
          <svg className="absolute inset-0 w-full h-full drop-shadow-[0_0_15px_rgba(239,68,68,0.2)]" viewBox="0 0 100 100">
            {/* Watch Ticks */}
            {ticks}
          ...
          <div className="flex flex-col items-center justify-center">
            <div className="font-mono text-5xl md:text-7xl tracking-tighter font-light text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]">
              {formatTime(timeLeft)}
            </div>
            {!isActive && (
              <div className="text-[9px] uppercase tracking-[0.3em] text-white/30 mt-2 absolute bottom-16">
                {lang === 'en' ? 'Adjust Time' : 'समय बदलें'}
              </div>
            )}
          </div>
        </div>
